...
class LatentDiffusion(DDPM):
    ...
    def shared_step(self, batch, **kwargs):
        ...
        clip_target = batch.get('image_embed', image_raw)
        ...
        # print(c.shape)
        if self.return_cond:
            loss, cc = self(x, c, label, clip_target)
            return loss, cc
        else:    
            loss = self(x, c, label, clip_target)
            return loss
    ...
    def forward(self, x, c, label, image_raw, *args, **kwargs):
        ...
        if self.clip_tune:
            if torch.is_tensor(image_raw):
                image_embeds = image_raw.to(device=re_latent.device, dtype=re_latent.dtype)
            else:
        ...
        # loss_dict.update({f'{prefix}/loss_cls': loss_cls})
            ...
            for metric_name, metric_value in getattr(
                    self.cond_stage_model, 'clip_loss_details', {}).items():
                loss_dict.update({f'{prefix}/{metric_name}': metric_value})
        if self.cls_tune:
            pre_cls = self.cond_stage_model.get_cls(re_latent)
            loss_cls = self.cls_loss(label, pre_cls)
            # image_embeds = self.image_embedder(image_raw)
    ...
    def configure_optimizers(self):
        ...
        if self.train_cond_stage_only:
            ...
            freeze_things_eeg = bool(
                getattr(self.cond_stage_model, 'is_things_eeg2', False)
                and getattr(self.cond_stage_model, 'things_freeze_eeg_encoder', False))
            cond_parms = [p for n, p in self.named_parameters() 
                    if ('attn2' in n or 'time_embed_condtion' in n or 'norm2' in n)
                    and not (freeze_things_eeg and n.startswith('cond_stage_model.mae.'))]
            # cond_parms = [p for n, p in self.named_parameters() 
                    # if 'time_embed_condtion' in n]
            # cond_parms = []
            
            if freeze_things_eeg:
                for param in self.cond_stage_model.mae.parameters():
                    param.requires_grad = False
                cond_stage_params = [
                    param for name, param in self.cond_stage_model.named_parameters()
                    if not name.startswith('mae.')]
                params = cond_stage_params + cond_parms
                print(f'THINGS-EEG2 pretrained EEG backbone frozen: '
                      f'{len(cond_stage_params)} conditioner tensors remain trainable')
            else:
                params = list(self.cond_stage_model.parameters()) + cond_parms
            if getattr(self.cond_stage_model, 'is_things_eeg2', False):
                original_count = len(params)
                params = list({id(param): param for param in params}.values())
                print(f'THINGS-EEG2 optimizer parameter deduplication: '
                      f'{original_count} entries -> {len(params)} unique tensors')
        
            for p in params:
                p.requires_grad = True

            eeg_lr_scale = float(getattr(
                self.cond_stage_model, 'things_eeg_encoder_lr_scale', 1.0))
            if (getattr(self.cond_stage_model, 'is_things_eeg2', False)
                    and not freeze_things_eeg and eeg_lr_scale != 1.0):
                if eeg_lr_scale <= 0:
                    raise ValueError('things_eeg_encoder_lr_scale must be positive')
                eeg_param_ids = {
                    id(param) for param in self.cond_stage_model.mae.parameters()}
                eeg_params = [param for param in params if id(param) in eeg_param_ids]
                other_params = [param for param in params if id(param) not in eeg_param_ids]
                params = [
                    {'params': other_params, 'lr': lr},
                    {'params': eeg_params, 'lr': lr * eeg_lr_scale},
                ]
                print(f'THINGS-EEG2 EEG backbone LR scale: {eeg_lr_scale:g} '
                      f'({lr * eeg_lr_scale:g} vs {lr:g})')
