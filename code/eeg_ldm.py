...
def main(config):
    ...
    if config.dataset == 'EEG':
        ...
        eeg_latents_dataset_train, eeg_latents_dataset_test = create_myEEGImageNetDataset(args=config, split_ratio=config.split_ratio,
                                                                                          image_transform=[
                                                                                              img_transform_train,
                                                                                              img_transform_test])
        ...
        num_voxels = config.eeg_data_len
...
def get_args_parser():
    ...
    # EEG-ImageNet数据集参数
    parser.add_argument('--dataset_dir', default=r'/home/mahui/Dataset/EEG-ImageNet-Dataset', type=str,
                        help='path to EEG-ImageNet dataset')
    parser.add_argument('--data_file', default='EEG-ImageNet_1.pth', type=str, help='file of EEG-ImageNet dataset')
    parser.add_argument('--subject', default=0, type=int, help='subject index')  # 选哪些人的EEG，-1默认全选
    parser.add_argument('--granularity', default='all', type=str, )
    parser.add_argument('--eeg_normorlize', action='store_true', default=False)  # 是否启用自定义eeg归一化
